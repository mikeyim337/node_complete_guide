...
exports.getProducts = (req, res, next) => {
  const page = +req.query.page || 1; //access the query parameter using express.js *page bc i named the query parameter there  PLUS in fron turns it into an integer
  let totalItems;
  ...
  Product.find()
    ...
    .then((numProducts) => {
      ...
      return Product.find()
        ...
        .limit(ITEMS_PER_PAGE); //limit the current page to only fetch the item limit
    })
    .then((products) => {
...
exports.getInvoice = (req, res, next) => {
  ...
  Order.findById(orderId)
    .then((order) => {
      ...
      // file.pipe(res); //response is a writable stream. Data will be downloaded by the browser step by step, so node doesnt have to preload the data
    })
    .catch((err) => next(err));
};
